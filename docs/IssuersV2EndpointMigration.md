...
| `token_endpoint` | string (optional) | Optional in config — the application computes and overwrites this value at runtime. |
...
- **V2** ignores most extra fields but will include the computed `token_endpoint` in its response.
...
| **Required fields** | `issuer_id`, `protocol`, `display`, `client_id`, `token_endpoint`, `client_alias`, `qr_code_type`, `enabled`, `credential_issuer_host`. config-only responses but `token_endpoint` is computed and injected at runtime. |
