...
public class VcSdJwtCredentialFormatHandler implements CredentialFormatHandler {
    ...
    private Map<String, Object> extractSdClaimsV2(SDJWT sdJwt) {
        ...
        resolveDisclosures(payload, "", Collections.emptyList(),
                digestToDisclosure, digestToDisclosureB64, pathToDisclosures);
    ...
    private void resolveDisclosures(
            Object value,
            String path,
            List<String> parentDisclosures,
            Map<String, Disclosure> digestToDisclosure,
            Map<String, String> digestToDisclosureB64,
                ...
                if (item instanceof Map) {
                    ...
                    if (mapItem.size() == 1 && mapItem.containsKey(KEY)) {
                        ...
                        if (disclosure != null) {
                            ...
                            resolveDisclosures(disclosure.getClaimValue(), currentPath,
                                    currentDisclosures, digestToDisclosure, digestToDisclosureB64, pathToDisclosures);
                        }
                        continue;
                    }
                }
                resolveDisclosures(item, currentPath, parentDisclosures,
                        digestToDisclosure, digestToDisclosureB64, pathToDisclosures);
            }
            return;
        }
        ...
        if (value instanceof Map) {
            ...
            if (sdDigestsObj instanceof List) {
                for (Object digestObj : (List<Object>) sdDigestsObj) {
                    ...
                    resolveDisclosures(disclosure.getClaimValue(), fullPath,
                            currentDisclosures, digestToDisclosure, digestToDisclosureB64, pathToDisclosures);
                }
            }
            ...
            for (Map.Entry<String, Object> entry : map.entrySet()) {
                ...
                resolveDisclosures(entry.getValue(), fullPath, parentDisclosures,
                        digestToDisclosure, digestToDisclosureB64, pathToDisclosures);
            }
        }
    }
